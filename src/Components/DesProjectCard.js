...
const DesProjectCard = (project, linkRoute) => {
    ...
    return (
        <div className="col-12 col-md-6 col-lg-3 p-0 my-3 my-lg-0" onMouseOver={() => setState(" d-flex animate__fadeIn")} onMouseLeave={() => setState(" animate__fadeOut d-flex")}>
            <div className="card border-0 rounded-0 d-flex justify-content-center">
                ...
                <div className={"card-img-overlay text-white rounded-0 p-2 animate__animated flex-column justify-content-center align-items-center" + state} >
                    ...
                    <Link to={`/${project.project.route}`} className="btn btn-outline-dark text-center rounded-sm">ver más...</Link>
                </div>
            </div>
        </div>
    )
